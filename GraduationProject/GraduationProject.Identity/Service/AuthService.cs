...
namespace GraduationProject.Identity.Service
{
    public class AuthService : IAuthService
    {
        ...
        private async Task<JwtSecurityToken> CreateJwtToken(ApplicationUser user)
        {
            ...
            var claims = new[]
            {
                ...
                new Claim(JwtRegisteredClaimNames.Name, user.NameEnglish),
    ...
    public class JwtTokenLifetimeManager : IJwtTokenLifetimeManager
    {
        private static readonly ConcurrentDictionary<string, DateTime> DisavowedSignatures = new();
        ...
        public void SignOut(JwtSecurityToken securityToken)
        {
            if (securityToken is JwtSecurityToken token)
                DisavowedSignatures.TryAdd(token.RawSignature, token.ValidTo);
